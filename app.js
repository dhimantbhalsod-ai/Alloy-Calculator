...
(function () {
  ...
  function renderWasteEntries(count) {
    ...
    document.querySelectorAll('.waste-gross-input, .waste-bag-input').forEach(input => {
      input.addEventListener('input', () => {
        updateUsableWasteForEntry(input.dataset.index);
      });
    });
  }
  ...
  function updateAllUsableWaste() {
    ...
    for (let i = 0; i < count; i++) updateUsableWasteForEntry(i);
  }
  ...
  function calculateAlloyWeights(totalWeight, desiredSilverPct, wasteEntries = []) {
    ...
    if (totalWasteWeight > totalWeight) {
      ...
      guidance = 'WARNING: Total waste weight exceeds your order weight. Reduce waste quantities accordingly.';
      totalWasteWeight = totalWeight;
      totalPureSilverFromWaste *= scaleFactor;
      totalCopperFromWaste *= scaleFactor;
      wasteEntries = wasteEntries.map(e => ({ weight: e.weight * scaleFactor, purity: e.purity }));
    }
  ...
  function colorPurityIndicator(element, actualPct, minPct, maxPct) {
    if (actualPct >= minPct - 0.01 && actualPct <= maxPct + 0.01) {
      element.style.color = '#4a8a5a';
    } else {
      element.style.color = '#c0534f';
    }
  }
  ...
  form.addEventListener('submit', e => {
    ...
    if (isNaN(rawWeight) || rawWeight <= 0) return showError('Total weight must be greater than 0');
    if (isNaN(minSilverPct) || minSilverPct < 1 || minSilverPct > 98.5) return showError('Min silver percentage must be between 1% and 98.5%');
    if (maxSilverPct < minSilverPct) return showError('Max silver % must be ≥ Min silver %');
    if (maxSilverPct > 98.5) return showError('Max silver percentage cannot exceed 98.5%');
    ...
    if (usingWaste) {
      ...
      if (wasteEntries.length === 0) return showError('Enter valid waste weight and purity for at least one waste alloy');
      for (const entry of wasteEntries) {
        if (entry.purity <= 0 || entry.purity > 98.5) return showError('Waste purity must be between 1% and 98.5%');
      }
    }
    ...
    // NO range warning — just show the result as-is (user request: remove warning)
    const combinedGuidance = result.guidance; // only waste overflow guidance kept
    ...
    lastCalcState = {
      ...
      usingWaste, silverPurity, copperPurity, silverInCopper
    };
    ...
    saveToHistory({
      ...
      pureSilverFromWaste: result.pureSilverFromWaste,
      pureSilverFromSilver,
      pureSilverFromCopper,
      totalPureSilver,
      actualPct,
      guidance: combinedGuidance,
      silverPurity, copperPurity,
      // Placeholders for adjusted values (updated when steps 2/3 are done)
      actualSilverUsed: result.weightOfSilver,
      actualCopperUsed: result.freshCopperNeeded,
      finalPct: actualPct,
      wasAdjusted: false
    });
  });
  ...
  function displayResults(result, usingWaste, verification, minPct, maxPct, silverPurity, copperPurity) {
    ...
    if (usingWaste && result.wasteUsed > 0) {
      ...
      if (result.wasteEntries && result.wasteEntries.length > 1) {
        resWastePurity.textContent = 'purities: ' + result.wasteEntries.map(e => e.purity.toFixed(1) + '%').join(', ');
  ...
  btnStep2.addEventListener('click', () => {
    if (!lastCalcState) return showError('Run the ideal calculation first');

    const actualSilverRaw = parseFloat(actualSilverIn.value);
    if (isNaN(actualSilverRaw) || actualSilverRaw <= 0) return showError('Enter a valid actual silver weight');
  ...
  btnStep3.addEventListener('click', () => {
    if (!lastCalcState || !lastCalcState.actualSilver) return showError('Complete Step 2 first');

    const actualCopperRaw = parseFloat(actualCopperIn.value);
    if (isNaN(actualCopperRaw) || actualCopperRaw < 0) return showError('Enter a valid actual copper weight');

    let actualCopper = toGrams(actualCopperRaw);
    const { actualSilver, wasteUsed, pureSilverFromWaste: psfWaste, minSilverPct, maxSilverPct, totalWeight, usingWaste, silverPurity, silverInCopper } = lastCalcState;
    ...
    if (!inRange) {
      wasAdjusted = true;
      let targetPct = actualPct > maxSilverPct + 0.01 ? maxSilverPct : minSilverPct;
    ...
    if (wasAdjusted) {
      ...
      adjCopperSublabel.textContent = 'your copper metal';
    }
    ...
    adjWeightDiff.textContent = (weightDiff >= 0 ? '+' : '−') + formatWeight(Math.abs(weightDiff));
  ...
  function renderHistory() {
    ...
    history.forEach((entry, i) => {
      ...
      const purityDisplay = entry.maxSilverPct
        ? `${entry.minSilverPct.toFixed(1)}–${entry.maxSilverPct.toFixed(1)}%`
        : `${(entry.desiredSilverPct || entry.minSilverPct || 0).toFixed(2)}%`;
      ...
      const actualPctFormatted = entry.actualPct ? entry.actualPct.toFixed(2) + '%' : '—';

      // Compact summary row
      const silverUsed = entry.actualSilverUsed || entry.silverWeight || 0;
      const copperUsed = entry.actualCopperUsed || entry.freshCopperWeight || 0;

      // Expanded details
      const idealSilver = entry.idealSilver || entry.silverWeight || 0;
      const idealCopper = entry.idealCopper || entry.freshCopperWeight || 0;

      let wasteDetailHtml = '';
      if (entry.usingWaste && entry.wasteUsed > 0) {
        wasteDetailHtml = `
          <div class="hist-detail-row">
            <span class="hist-label">Waste used</span>
            <span class="hist-value waste-val">${entry.wasteUsed.toFixed(2)} g</span>
          </div>`;
        if (entry.wasteEntries && entry.wasteEntries.length > 0) {
          entry.wasteEntries.forEach((w, wi) => {
            wasteDetailHtml += `
              <div class="hist-detail-row hist-sub">
                <span class="hist-label">  Waste ${wi + 1}</span>
                <span class="hist-value waste-val">${w.weight.toFixed(2)} g @ ${w.purity.toFixed(1)}%</span>
              </div>`;
          });
        }
      }
      ...
      let verifyHtml = '';
      if (entry.pureSilverFromSilver !== undefined) {
        verifyHtml = `
          <div class="hist-section-title">Verification</div>
          <div class="hist-detail-row">
            <span class="hist-label">Silver from silver</span>
            <span class="hist-value">${(entry.pureSilverFromSilver || 0).toFixed(2)} g</span>
          </div>
          <div class="hist-detail-row">
            <span class="hist-label">Silver from copper</span>
            <span class="hist-value">${(entry.pureSilverFromCopper || 0).toFixed(2)} g</span>
          </div>`;
        if (entry.usingWaste && entry.pureSilverFromWaste) {
          verifyHtml += `
            <div class="hist-detail-row">
              <span class="hist-label">Silver from waste</span>
              <span class="hist-value">${entry.pureSilverFromWaste.toFixed(2)} g</span>
            </div>`;
        }
        verifyHtml += `
          <div class="hist-detail-row hist-total">
            <span class="hist-label">Total pure silver</span>
            <span class="hist-value">${(entry.totalPureSilver || 0).toFixed(2)} g</span>
          </div>`;
      }

      card.innerHTML = `
        <div class="hist-compact" data-index="${i}">
          <div class="hist-compact-left">
            <div class="hist-compact-weight">${displayWeight}</div>
            <div class="hist-compact-metals">
              <span class="hist-chip silver-chip">Ag ${silverUsed.toFixed(1)}g</span>
              <span class="hist-chip copper-chip">Cu ${copperUsed.toFixed(1)}g</span>
            </div>
          </div>
          <div class="hist-compact-right">
            <div class="hist-compact-purity">${actualPctFormatted}</div>
            <div class="hist-compact-date">${formatDate(entry.timestamp)}</div>
          </div>
          <svg class="hist-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
        <div class="hist-expanded" style="display:none;">
          <div class="hist-detail-row">
            <span class="hist-label">Order weight</span>
            <span class="hist-value">${displayWeight}</span>
          </div>
          <div class="hist-detail-row">
            <span class="hist-label">Target purity</span>
            <span class="hist-value">${purityDisplay}</span>
          </div>
          <div class="hist-section-title">Ideal Weights</div>
          <div class="hist-detail-row">
            <span class="hist-label">Silver (ideal)</span>
            <span class="hist-value silver-val">${idealSilver.toFixed(2)} g</span>
          </div>
          <div class="hist-detail-row">
            <span class="hist-label">Copper (ideal)</span>
            <span class="hist-value copper-val">${idealCopper.toFixed(2)} g</span>
          </div>
          ${wasteDetailHtml}
          <div class="hist-section-title">Used Weights</div>
          <div class="hist-detail-row">
            <span class="hist-label">Silver (used)</span>
            <span class="hist-value silver-val">${silverUsed.toFixed(2)} g</span>
          </div>
          <div class="hist-detail-row">
            <span class="hist-label">Copper (used)</span>
            <span class="hist-value copper-val">${copperUsed.toFixed(2)} g</span>
          </div>
          ${verifyHtml}
          <div class="hist-detail-row hist-purity-final">
            <span class="hist-label">Final purity</span>
            <span class="hist-value">${actualPctFormatted}</span>
          </div>
          <button class="btn-delete-history" data-index="${i}">
    ...
    historyList.querySelectorAll('.hist-compact').forEach(compact => {
      compact.addEventListener('click', () => {
        const card = compact.parentElement;
        const expanded = card.querySelector('.hist-expanded');
        const chevron = compact.querySelector('.hist-chevron');
        const isOpen = expanded.style.display !== 'none';
        expanded.style.display = isOpen ? 'none' : 'block';
        chevron.classList.toggle('hist-chevron-open', !isOpen);
      });
    });
